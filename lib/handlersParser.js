...
Handler.prototype.functions = function (direct) {
  var handler = this;
	
	var functions = require(this.file)[this.funcName]
  ...
  var wrapedFunctions = [];
	...
	functions.forEach(function(func) {
		var promFunc = Promise.method(func);
		wrapedFunctions.push(
			function (req, res, next) {
				return utils.pTimeout(promFunc(req, res, next), apiTimeout, 'API Request timed out')
	...
	return wrapedFunctions;
};

Handler.prototype.getRegisterParams = function(){
	return [this.path].concat(this.functions())
}
